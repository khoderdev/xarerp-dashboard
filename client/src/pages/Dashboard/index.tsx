...
//   const viewFinancialAnalyticsRoles = ['Administration', 'Financial'];
//   const viewStoresAnalyticsRoles = ['Administration'];
...
//               &&
//               <C.ContainerSections>
//                 <C.TitleSections>Sales in the last
//                   <C.BtnArea>
//                     <button
      ...
      <C.Graphics>
        {data.result &&
          <>
            {viewSalesAnalyticsRoles.includes(auth.user?.position ?? '') &&
              <C.ContainerSections>
                <C.TitleSections>Orders / Transfers in the last
                  <C.BtnArea>
                    <button
                      className={targetDate === 7 ? 'active' : ''}
                      onClick={() => setTargetDate(7)}
            ...
            {viewStoresAnalyticsRoles.includes(auth.user?.position ?? '') &&
              <C.ContainerSections>
                <C.TitleSections>Profit by store type in the last
                  <C.BtnArea>
                    <button
                      className={targetDate === 7 ? 'active' : ''}
                      onClick={() => setTargetDate(7)}
            ...
            {viewProductsAnalyticsRoles.includes(auth.user?.position ?? '') &&
              <C.ContainerSections>
                ...
                <C.AverageTicket>
                  ...
                  <strong>$ {data.result.products.averageTotalTicket}</strong>
                </C.AverageTicket>
              </C.ContainerSections>
            }
            ...
            {viewFinancialAnalyticsRoles.includes(auth.user?.position ?? '') &&
              <C.ContainerSections>
                <C.TitleSections>Manual entries in the last
                  <C.BtnArea>
                    <button
                      className={targetDate === 7 ? 'active' : ''}
                      onClick={() => setTargetDate(7)}
                ...
                <C.SalesArea>
                  <C.ResumeBalanceArea>
                    <ResumeBalance value={data.result.financial.difference} type="Balance" />
                    <ResumeBalance value={data.result.financial.totalEntries} type="Entry" />
                    <ResumeBalance value={data.result.financial.totalOutputs} type="Exit" />
                  </C.ResumeBalanceArea>
                  <C.SalesDaily>
                    <BarChartPositiveAndNegativeXR
                      data={data.result.financial.list}
